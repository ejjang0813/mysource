...
public class StackNumberServlet extends HttpServlet implements Servlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//1. HttpSession조회
		HttpSession session = request.getSession();//요청한 사람의 세션을 달라!
		//2. 로그인 여부 체크 로그인 안햇으면 login_form.jsp로 이동.
		if(session.getAttribute("loginMember") == null) {
			request.setAttribute("errorMessage", "숫자 누적은 로그인 해야합니다.");
			request.getRequestDispatcher("/login/login_form.jsp").forward(request, response);
			return;
		}
		
		int num = (int)(Math.random()*1000);//누적할 새 숫자.(랜덤숫자 0~999)
		/*
		 * 누적할 숫자들을 저장할 List를 session에서 조회.
